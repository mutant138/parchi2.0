...
const EditInvoice = () => {
  ...
  useEffect(() => {
    function addActionQty() {
      if (formData?.items?.length === 0 || products.length === 0) {
        return;
      }
  ...
  async function onEditInvoice() {
    try {
      ...
      await updateDoc(
        doc(db, "companies", companyDetails.companyId, "invoices", id),
        payload
      );
      ...
      <div className="bg-white p-6 rounded-lg shadow-lg">
        ...
        <div className="mt-6 flex justify-end">
          <div className="flex gap-2">
            ...
            >
              <span className="text-lg">+</span> Edit Invoice
            </button>
          </div>
        </div>
      </div>
