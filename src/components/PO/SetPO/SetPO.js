  ...
  const SetPO = () => {
          ...
          try {
            ...
            const productsData = querySnapshot.docs.map((doc) => {
              ...
              if (data.discountType) {
                discount = ((data.purchasePrice <= 0 ? +data.sellingPrice : +data.purchasePrice ) / 100) * data.discount;
                console.log("discount", discount)
             
              }
              const netAmount = (data.purchasePrice <= 0 ? +data.sellingPrice : data.purchasePrice ) - discount;
              console.log("price", netAmount,data.purchasePrice, data.sellingPrice)
      ...
      async function onSetPo() {
        try {
          ...
          if (poId) {
            await updateDoc(
              doc(db, "companies", companyDetails.companyId, "purchases", poId),
              payload
            );
          } else {
            await addDoc(
